...
- `server/`의 `npm test`: 12개 통과. 실제 마이그레이션 SQL을 인메모리 SQLite에 적용하고 외래키·배치 트랜잭션을 활성화했다. 이 테스트는 원격 D1 동시성 시험을 대신하지 않는다.
...
2. **경쟁 공정성:** 서버가 문장 배정·오염·완료를 전부 소유하지 않는다. 문장 풀·누적 속도 검사는 임의 텍스트와 과속 일부를 차단하지만, 클라이언트의 완료 신고·불붙음 보너스·피격 적용을 모두 증명하지 못한다. 서버 권한으로 게임 상태를 옮긴 뒤 랭크전 조작 회귀 검증이 필요하다.
